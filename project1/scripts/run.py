...
def I_do_it_all_and_I_try_to_do_it_good_REG_LOG_REG(degree, lambdas_, k_fold=10):
    ...
    for lambda_ in lambdas_:
        ...
        if (2*accuracy + F1 > best_heuristique):
            best_w = w
            best_TP, best_TN, best_FP, best_FN = implementations.stats(
                y_test, y_pred)
            best_lambda = lambda_
            best_F1 = F1
...
def trial_high_dimension():
    ...
    y[y == -1] = 0
    tX_completed = implementations.build_poly(tX_completed,9)
    tX_test_completed = implementations.build_poly(tX_test_completed, 9)
    tX_train = implementations.build_poly(tX, 9)
    i = 0
    accuracy = 0
    indices = implementations.build_k_indices(len(y), 9)
    x_train, y_train, x_test, y_test = implementations.cross_validation_split(
        y, tX_completed, indices, 0)
    while(accuracy < 0.75):
        i += 1
        gamma = 5/10**i
        w, l = implementations.get_w_loss(y_train, x_train, 1, 0, 3000, gamma)
        y_pred = predict_labels(w, x_test)
        #F1 = implementations.f1_score(y_test, y_pred)
        # print(F1)
        matches = [i for i, j in zip(y_pred, y_test) if i == j]
